...
var Fits=Backbone.Model.extend({idAttribute:"_id",urlRoot:"/fits/"}),FitsCollection=Slickback.Collection.extend({model:Fits,url:function(){return"/fits/"+this.logID+"/"+this.viewID},printURL:function(){return"/print/"+this.logID+"/"+this.viewID},initialize:function(a,b){this.logID=a;this.viewID=b||""}});
...
this.collection.viewID=this.fView.id;this.monitor()},checkStart:function(){0<this.collection.length&&($("#noGrid").hide(),this.collection.off("reset",this.checkStart),this.startGrid())},startGrid:function(){this.$el.show();$("#printGrid").on("click",this.printGrid);columns=this.fView.columns();columns[0].formatter=this.groupFormatter;this.grid=new Slick.Grid("#grid",this.collection,columns,{editable:!0,enableCellNavigation:!0,formatterFactory:Slickback.BackboneModelFormatterFactory});this.grid.setSelectionModel(new Slick.RowSelectionModel);
this.grid.registerPlugin(new Slick.RowMoveManager);this.grid.onClick.subscribe(this.clickGrid);this.grid.onContextMenu.subscribe(this.contextMenu);$("#groupMenu").on("click",this.contextClick);this.collection.on("change",this.saveChanges);this.grid.onBeforeEditCell.subscribe(function(){q.pause("fetchFits")});this.grid.onBeforeCellEditorDestroy.subscribe(function(){q.unpause("fetchFits")});this.collection.on("reset",this.resetGrid);this.resizeGrid=this.resize;$(window).on("resize",this.resizeGrid);
this.resizeGrid();this.resetGrid()},groupFormatter:function(a,b,c,e,d){value=d.get(e.field);return d.has("childrenExpand")?d.get("childrenExpand")?"<div class='toggle collapse'>"+value+"</div>":"<div class='toggle expand'>"+value+"</div>":d.get("group")?"<span class='groupSpacer'></span>"+value:value},clickGrid:function(a,b){if($(a.target).hasClass("toggle")){var c=this.collection.at(b.row);c.get("childrenExpand")?c.set({childrenExpand:!1}):c.set({childrenExpand:!0});a.stopImmediatePropagation();
this.resetGrid()}},contextMenu:function(a){$("#groupMenu").hide();var b=this.grid.getSelectedRows(),c=this.grid.getCellFromEvent(a);-1!=$.inArray(c.row,b)&&(a.preventDefault(),this.collection.at(c.row),a.preventDefault(),$("#groupMenu").data("row",c.row).css("top",a.pageY).css("left",a.pageX).show(),$("body").one("click",function(){$("#groupMenu").hide()}))},contextClick:function(a){var a=$(a.target).attr("data"),b=this.grid.getSelectedRows();"group"==a?this.groupRows(b):"ungroup"==a&&this.removeRowGroups(b)},
...
1),this.prevCount=this.collection.length)},saveChanges:function(a){a.save()},printGrid:function(){window.open(this.collection.printURL())},monitor:function(){q.monitor("fetchFits",12E4)},onClose:function(){q.remove("fetchFits");this.collection.off("change");this.collection.off("reset");this.viewSelector.$el.off();this.viewSelector.close();$("#printGrid").off();$(window).off("resize");$("#groupMenu").off();this.grid&&(this.grid.onBeforeEditCell.unsubscribe(),this.grid.onBeforeCellEditorDestroy.unsubscribe(),
this.grid.onClick.unsubscribe(),this.grid.onContextMenu.unsubscribe(),delete this.grid);$("#grid").off()}});
