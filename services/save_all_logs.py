...
for al in aLogs:
    ...
    print "Retrieving information for log ID: "+str(al["_id"])
    mylog = db.logs.find_one({"_id": al["_id"]}, {"project":1, "utdate":1, "instrument":1})
    ...
    f = urllib2.urlopen("http://observinglogs/save/"+str(al["_id"])+"/"+str(fitsview["_id"]))
    #time.sleep(5)
